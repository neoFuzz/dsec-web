...
export class DSecGame {
    ...
    getCyclicPannedCameraPositionAndOrientation(vertex2D, rotateZ, xA, rotationXMatrix, yA, mode) {
        let base = 300.0;
        let {x1, x2, y1, y2, count} = this.panCheck();
        const x3 = x1 - x2;
        const y3 = y1 - y2;
        let xDiv = x2 + x3 / 2.0;
        let yDiv = y2 + y3 / 2.0;
    ...
    /**
     * Finds players the camera should pan over.
     *
     * @private
     * @returns {{y1: number, count: number, x1: number, y2: number, x2: number}}
     */
    panCheck() {
        let x1 = -1.0E10;
        let x2 = 1.0E10;
    ...
    awardEstimatedScores() {
        ...
        let stat4 = 0.0;
        if (dsector.DSReference.dsecMainSetupWindow.playMode() === dsector.DSecMainSetupWindow.TEAMS) {
            this.teamsCalculations(stat1, stat2, stat4, stat3);
        } else {
            DSecGame.hostileCalculations(stat1, stat2, stat3, stat4);
        }
    }
     ...
     */
    static hostileCalculations(stat1, stat2, stat3, stat4) {
        let rndnum;
        let counter = 0;
        stat1 = 0.0;
        stat2 = 0.0;
     ...
     * @param {number} stat4 the stat 4.
     */
    teamsCalculations(stat1, stat2, stat4, stat3) {
        let rndnum;
        let team = null;
     ...
     * @type {number}
     */
    static scoreCreditMultiplier = 7.0;
    /**
     ...
     * @type {number}
     */
    static tankSpeedRelativeToMissiles = 1.65;
    /**
