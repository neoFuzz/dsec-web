 ...
 * @license  AGPLv3
 */
export class DSecShoppingScreen {
    constructor() {
    ...
    drawWindow() {
        if (this.currentShopper != null) {
            ...
            this.window = dsector.DSReference.gui.addWindow$name$style$title$x$y$w$h$v(
                "SHOP", CWSYSTEM.CWWindowStyles.ROUNDED, null, x, y, w, 470, true);
            ...
            this.window.addTextBlock("", currentName, 5 + margin, 35, font,
                CWSYSTEM.CWColor.__white(), 999);
            this.window.addTextBlock("", "Money " + this.currentShopper.credits, margin + 88,
                35, font, CWSYSTEM.CWColor.__white(), 999);
            let refRound = 1;
            if (dsector.DSReference.dsecGame.currentRound() > 0) {
                refRound = dsector.DSReference.dsecGame.currentRound() + 1;
            }
            this.window.addTextBlock("", "Round " + refRound + " of " +
                dsector.DSReference.dsecMainSetupWindow.numberOfRounds(), margin + 160, 35,
                font, CWSYSTEM.CWColor.__white(), 999);
            ...
            for (let j = 1; j <= 7; ++j) {
                ...
                if (checked) {
                    if (j < 7) {
                        ...
                        this.window.addTextBlock("", "Items", 5 + margin, 25 + baseline,
                            font, CWSYSTEM.CWColor.__white(), 999);
                    }
                    baseline += 16;
                    h = 0;
            ...
            for (const weaponSpec of arrayList) {
                switch (weaponSpec.specificationID) {
                    ...
                    case dsector.PreBuiltWeaponSpecifications.SPEED_UPGRADE_2:
                        break;
                    default:
                        if (weaponSpec.portNumber >= 1 &&
                            weaponSpec.portNumber <= 7 &&
                            weaponSpec.price > 1200 &&
                            Math.random() < 0.05) {
                            const rndNumber = Math.random();
                            if (rndNumber < 0.04) {
                                CWSYSTEM.CWSReference.alertManager.messageQueued("Shopping special! 75% off " +
                                    weaponSpec.fullName + ", " + ((weaponSpec.price * 0.25) | 0) +
                                    " before shopping card discount");
                                DSecShoppingScreen.itemsOnSpecial.push(new dsector.ObjectPair(weaponSpec, 75));
                            } else if (rndNumber < 0.25) {
                                CWSYSTEM.CWSReference.alertManager.messageQueued("Shopping special! " +
                                    weaponSpec.fullName + " half price at " + ((weaponSpec.price * 0.5) | 0) +
                                    " before shopping card discount");
                                DSecShoppingScreen.itemsOnSpecial.push(new dsector.ObjectPair(weaponSpec, 50));
                            } else if (rndNumber < 0.3) {
                                CWSYSTEM.CWSReference.alertManager.messageQueued("Supply of " +
                                    weaponSpec.fullName + " extremely low. Offered only at quadruple price of " +
                                    weaponSpec.price * 2 + " before shopping card discount");
                                DSecShoppingScreen.itemsOnSpecial.push(new dsector.ObjectPair(weaponSpec, -300));
                            } else if (rndNumber < 0.45) {
                                CWSYSTEM.CWSReference.alertManager.messageQueued("Supply of " +
                                    weaponSpec.fullName + " low. Offered only at double price of " +
                                    weaponSpec.price * 2 + " before shopping card discount");
                                DSecShoppingScreen.itemsOnSpecial.push(new dsector.ObjectPair(weaponSpec, -100));
                            } else {
                                CWSYSTEM.CWSReference.alertManager.messageQueued("Shopping special! 25% off " +
                                    weaponSpec.fullName + ", " + ((weaponSpec.price * 0.75) | 0) +
                                    " before shopping card discount");
